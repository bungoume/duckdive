...
`package.json#packageManager` pins the pnpm version and the hash of its tarball, so `pnpm` resolves to that exact release through Corepack (0.34.7 or newer for pnpm 12) or pnpm's own version management. `pnpm-workspace.yaml` sets a 7-day cooldown (`minimumReleaseAge`): a version published more recently is never installed, transitive dependencies included, and `pnpm-lock.yaml` pins every package by its integrity hash. pnpm also refuses a release whose publish evidence is weaker than an earlier one (`trustPolicy: no-downgrade`). `.github/SECURITY.md` describes how to report a vulnerability. `.github/renovate.json` opens update PRs with the same cooldown and keeps the GitHub Actions pinned to commit SHAs. Every push and pull request is scanned: CodeQL with the extended query suite, gitleaks for committed secrets, OSV against the lock file, Semgrep's pattern rules, and a dependency review of what the pull request adds; actionlint and zizmor check the workflows themselves.
...
Everything before the first wildcard is used as the S3 prefix, so put as much of the file name in the pattern as you can when a date folder holds files from many load balancers. Files whose names contain a timestamp (`_20260909T0105Z_`, as in ALB and NLB logs) are dropped when they fall outside the time range; such a source re-lists when the range changes, date tokens or not. Listing needs the `s3:ListBucket` permission; see `docs/iam-role-policy.json`. A key without wildcards is listed too (one request) so that its size and ETag feed the cache; when that listing is denied the key is used as typed. `https://` URLs support date tokens only.

The Template menu fills in the pattern and format for the usual layouts: ALB, NLB, CloudFront, CloudTrail, VPC Flow Logs (text and Parquet), WAF, Network Firewall, Route 53 Resolver, S3 server access logs, Kinesis Data Firehose, CloudWatch Logs exports and SSM session logs on AWS; a Cloud Logging sink on Google Cloud; a Logpush job on Cloudflare. Replace `<bucket>` and `<prefix>` with your own values. Cloud Storage and R2 are read through their S3-compatible endpoints, which the template's note names.

### Formats

Auto-detection goes by path and extension. Parquet, CSV and JSON are read with DuckDB's own readers (gzip and zstd included). Layouts of their own are recognised for ALB access logs, CloudFront standard logs, CloudTrail (`Records` expanded to one row per event), VPC Flow Logs (fields from the header line), S3 server access logs, WAF logs, Route 53 Resolver query logs, LTSV (labels exposed as `log.<label>`), CloudWatch Logs exports and plain text (one row per line). Google Cloud Logging exports and Cloudflare Logpush are formats too, chosen from the Template menu rather than detected, because their paths carry nothing to recognise them by. String time fields are parsed as ISO 8601, nginx/Apache format, or epoch seconds/milliseconds.

A flow log writes `-` in every field that does not apply to a record, so that is what the format reads as no value; without it a single such row makes DuckDB type the whole column as text.
...
"Field names" on the Data source page reads a log either under the names it delivers (the default) or under the names of the [OpenTelemetry semantic conventions](https://opentelemetry.io/docs/specs/semconv/). It covers the layouts duckdive recognises as a log family:

| Provider     | Layouts                                                                                                                        |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------ |
| AWS          | ALB access logs, CloudFront standard logs, S3 server access logs, CloudTrail, WAF, Route 53 Resolver query logs, VPC Flow Logs |
| Google Cloud | Cloud Logging exports (Load Balancing, Cloud Run, GKE and anything else a sink writes)                                         |
| Cloudflare   | Logpush, HTTP requests dataset                                                                                                 |

Parquet, CSV, JSON and the other generic formats keep the names they deliver: the file says how to parse the bytes but not which log family they are, so there is nothing to map. The choice is disabled for them.

The time field is `timestamp` under this naming, whatever the log called it. Fields whose parts the conventions name separately are split: `client:port` becomes `client.address` and `client.port` (at the last colon, so an IPv6 address survives), a request line becomes `http.request.method`, `url.full` and `network.protocol.version`, a URL becomes `url.scheme` / `url.path` / `url.query`, and `TLSv1.2` becomes `tls.protocol.name` + `tls.protocol.version`. Cloud Logging's `severity` becomes both `severity_text` and the `severity_number` the conventions use, and its `trace` becomes a bare `trace_id`.

Values are otherwise left as the log wrote them: `-` stays `-` and ALB's `-1` stays `-1`; CloudFront's percent-encoded URI, User-Agent and Referer are not decoded. A column that is derived rather than renamed is NULL when what it is derived from has no such part (a malformed ALB request line, `"- - -"`, gives no method and no URL).

Nothing is dropped. The conventions have no attributes for load balancers, CDNs, object storage access or cloud audit trails, so roughly half of each layout has no counterpart; those fields keep their own name under the vendor namespace of the service — `aws.alb.*`, `aws.cloudfront.*`, `aws.s3.*`, `aws.cloudtrail.*`, `aws.waf.*`, `aws.route53.*`, `aws.vpc.flow.*`, `gcp.*`, `cloudflare.*` — which is the extension point the conventions provide. The JSON families keep the whole record in `body` as well, and a Cloud Logging entry keeps its envelope in `log.record.original`, so a key the projection does not name is still reachable as `body.<key>` in the field sidebar.

How a layout is mapped depends on what is known about its columns before anything is read:

- ALB, CloudFront and S3 server access logs declare their columns here, so the projection is written against them directly.
- CloudTrail, WAF, Route 53 Resolver, Cloud Logging and Logpush are JSON, where keys differ between records and between files. They are read record by record and each attribute is pulled out by path, so a key a file lacks is NULL instead of an error. This binds without inferring a schema, which is cheaper than the generic JSON reader, not dearer.
- A flow log names its fields in its own header line, and which fields it has is chosen when the flow log is created. The mapping is applied to the columns the source turns out to have, after the view is created and its column names are read from the catalog; a field the mapping does not name keeps its own under `aws.vpc.flow.*`, so a custom format still reads. This is the one case that binds the reader twice.

Attributes outside the stable set (`http.request.size`, `tls.*`, `cloud.*`, `aws.*`, `gcp.*`, `destination.*`, `user.id`) may still change in a future version of the conventions; a rename here is a breaking change of this extension.
...
AWS log delivery sometimes writes gzip files as several concatenated members. DuckDB reads only part of such a file: it continues into the next member only when the read that filled its buffer was a full one, and otherwise ends the file at the member boundary without an error, so rows go missing silently. The diagnose report on the Data source page names the members of a file it is given. Convert wide ranges of such logs to Parquet (`scripts/alb-to-parquet.sh`) to avoid the problem entirely.
